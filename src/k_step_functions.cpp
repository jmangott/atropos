...
multi_array<double, 1> CalculateWeightX2(multi_array<Index, 1> vec_index1, mysys reaction_system, grid_info grid, Index mu)
{
...
void CalculateCoefficientsX2(multi_array<double, 2> &c2, multi_array<double, 2> &d2, const lr2<double> &lr_sol, blas_ops blas, const multi_array<double, 2> &xx2_shift, multi_array<Index, 1> vec_index1, mysys reaction_system, grid_info grid, Index mu)
{
    multi_array<double, 1> w_x2({grid.dx2});
...
void PerformKStep(vector<Index> sigma1, vector<Index> sigma2, lr2<double> &lr_sol, blas_ops blas, mysys reaction_system, grid_info grid, double tau)
{
    ...
    multi_array<Index, 1> n_xx1_reduced({grid.m1});
    vector<Index> vec_index1_coeff;
    multi_array<Index, 1> vec_index1_reduced({grid.m1});
    multi_array<Index, 1> vec_index1_zero({grid.m1});
    vector<double> lim_xx1_coeff;
    multi_array<double, 1> lim_xx1_reduced({grid.m1});
    ...
    for (std::vector<myreact *>::size_type mu = 0; mu < reaction_system.mu(); mu++)
    {
        ...
        for (Index i = 0; i < dxx1_coeff_mult; i++)
        {
            ...
            CalculateCoefficientsX2(c2, d2, lr_sol, blas, xx2_shift, vec_index1_zero, reaction_system, grid, mu);
