...
void internal_node<double>::Write(int ncid, int id_r_in,
  ...
  int varid_Q;
  // NOTE: netCDF stores arrays in row-major order (but Ensign column-major
  // order)
...
void cme_lr_tree::Read(const std::string fn) {
  ...
  root->child[0] =
      ReadHelpers::ReadNode(grp_ncid, "0", root, root->RankOut()[0]);

  NETCDF_CHECK(nc_inq_ncid(ncid, "1", &grp_ncid));
  root->child[1] =
      ReadHelpers::ReadNode(grp_ncid, "1", root, root->RankOut()[1]);
...
void WriteHelpers::WritePartitionStr(int ncid,
                                     const std::string partition_str) {
  size_t len = partition_str.size() + 1;
  NETCDF_CHECK(nc_put_att_text(ncid, NC_GLOBAL, "partition_str", len,
                               partition_str.data()));
}

void WriteHelpers::WriteSpeciesNames(
    int ncid, const std::vector<std::string> species_names) {
  int id_d;
  ...
  NETCDF_CHECK(nc_def_var(ncid, "species_names", NC_STRING, 1, &id_d,
                          &varid_species_names));
  NETCDF_CHECK(
...
void WriteHelpers::WriteGridParms(int ncid, const grid_parms grid) {
  ...
  std::vector<long long> binsize(std::begin(grid.binsize),
                                 std::end(grid.binsize));
...
void cme_lr_tree::PrintHelper(std::ostream &os,
                              cme_node const *const node) const {
  if (node->IsExternal()) {
    os << "external_node, id: " << node->id
       << ", species: " << node->grid.species << ", X.shape(): ("
       << ((cme_external_node *)node)->X.shape()[0] << ","
       << ((cme_external_node *)node)->X.shape()[1] << ")\n";
  } else {
    cme_lr_tree::PrintHelper(os, node->child[0]);
    cme_lr_tree::PrintHelper(os, node->child[1]);
    os << "internal_node, id: " << node->id
       << ", species: " << node->grid.species << ", rank_out: ("
       << ((cme_internal_node *)node)->RankOut()[0] << ","
...
std::vector<double>
cme_lr_tree::NormalizeHelper(cme_node const *const node) const {
...
std::string ReadHelpers::ReadPartitionStr(int ncid) {
  ...
  NETCDF_CHECK(
      nc_inq_attlen(ncid, NC_GLOBAL, "partition_str", &partition_str_len));
...
ReadHelpers::ReadPropensity(int ncid, const Index n_reactions) {
  ...
  for (Index mu = 0; mu < n_reactions; ++mu) {
    ...
    NETCDF_CHECK(nc_inq_varid(
        ncid, ("propensity_" + std::to_string(mu)).c_str(), &id_propensity));
...
cme_node *ReadHelpers::ReadNode(int ncid, const std::string id,
                                cme_internal_node *const parent_node,
                                const Index r_in) {
  int retval0, retval1;
  int grp_ncid0, grp_ncid1;
  ...
  if (retval0 or
      retval1) // NOTE: if retval0 is 1, then retval1 has also to be 1, due to
               // the binary tree structure of the netCDF file
  {
    // Initializing grid only for external nodes,
    // this prevents possible overflow for internal nodes
    grid.Initialize();
    child_node =
    ...
    child_node = new cme_internal_node(id, parent_node, grid, r_in, r_out,
                                       n_basisfunctions);
    child_node->Initialize(ncid);
    child_node->child[0] =
