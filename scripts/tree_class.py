...
class InternalNode(Node):
    ...
    def rankOut(self):
        return [self.Q.shape[0], self.Q.shape[1]]
...
class Tree:
    ...
    def __initialize(self, node: Node):
        ...
        if isinstance(node, InternalNode):
            if isinstance(self.__rank, dict):
                current_id = node.id
                r = self.__rank[current_id]
                r0 = self.__rank[current_id + 0]
                r1 = self.__rank[current_id + 1]
                if (r <= r0 * r1) and (r0 <= r * r1) and (r1 <= r * r0):
                    node.Q.resize((r0, r1, r))
                else:
                    raise Exception("Rank condition is not fulfilled")
            elif isinstance(self.__rank, np.ndarray):
                next_r_out = next(self.__rank_iter)
                node.Q.resize((next_r_out, next_r_out, node.parent.rankOut()[0]))
        ...
        if isinstance(node, ExternalNode):
            if isinstance(self.__rank, dict):
                current_id = node.id
                node.X.resize((node.grid.dx(), self.__rank[current_id]))
            elif isinstance(self.__rank, np.ndarray):
                node.X.resize((node.grid.dx(), node.parent.rankOut()[0]))
            node.propensity = self.__calculatePropensity(node)

        return

    def initialize(self, reaction_system: ReactionSystem, rank):
        # Test whether the dimension of `rank` is equal to n_internal_nodes
        if isinstance(rank, np.ndarray):
            if rank.size != self.n_internal_nodes:
                raise Exception(
                    "`rank.size` must be equal to the number of internal nodes"
                )
        ...
        if isinstance(rank, dict):
            if self.__rank[Id("0")] == self.__rank[Id("1")]:
                self.root.Q.resize((self.__rank[Id("0")], self.__rank[Id("1")], 1))
            else:
                raise Exception("Rank condition is not fulfilled")
        elif isinstance(rank, np.ndarray):
            self.__rank_iter = iter(rank)
            next_r_out = next(self.__rank_iter)
            self.root.Q.resize((next_r_out, next_r_out, 1))
        else:
            raise Exception("`rank` must be either a `dict` or a `np.ndarray`")
    ...
    def __write(self, node: Node, parent_dt: DataTree):
        if isinstance(node, ExternalNode):
            ...
            ds = self.__createDataset(node)
            ds["Q"] = (["n_basisfunctions", "r_out1", "r_out0"], node.Q.T)
    ...
    def write(self, fname: str = "input/input.nc"):
        ...
        ds["Q"] = (["n_basisfunctions", "r_out1", "r_out0"], self.root.Q.T)
