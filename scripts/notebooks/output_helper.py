...
class TimeSeries:
    ...
    def calculateMoments(self):
        n_moments = 2
        moments = [{name: np.zeros((self.__number_of_files)) for name in self.getSpeciesNames()} for _ in range(n_moments)]
        ...
        for i, filename in enumerate(self.__list_of_files):
            ...
            for j, n_j in enumerate(tree.grid.n):
                ...
                for m in range(n_moments):
                    moments[m][name][i] = np.dot(marginal_distribution[name], state ** (m + 1))
        return moments
