...
parser = argparse.ArgumentParser(
    ...
    usage='python3 scripts/input_generation/set_lambda_phage.py --partition "'
    + partition[0]
    + '" --rank 5 5',
...
# r_out = {Id("0"): args.rank[0], Id("1"): args.rank[1], Id("10"): args.rank[2], Id("11"): args.rank[3]}
# n_basisfunctions = np.ones(len(args.rank), dtype="int")
...
x0 = u[:, : tree.root.rankOut()[0]]
q = np.diag(s[: tree.root.rankOut()[0]])
x1 = vh[: tree.root.rankOut()[0], :].T
...
if partition_str is partition[3]:
    ...
    # x1_sum = np.array([x10_sum @ q1[:, :, i] @ x11_sum.T for i in range(r_out[Id("1")])])
else:
    initial_conditions.Q[0][:, :, 0] = q
    ...
    # x0_sum = np.array([x00_sum @ q0[:, :, i] @ x01_sum.T for i in range(r_out[Id("0")])])
    x1_sum = np.sum(x1, axis=0)
