...
void RemoveElement(InputIt first, InputIt last, OutputIt d_first,
                   const Index idx) {
...
void Matricize(const multi_array<T, d> &input, multi_array<T, 2> &output) {
  ...
  assert(shape[m] == output.shape()[1] &&
         prod(cols_shape) == output.shape()[0]);
...
#pragma omp parallel for
  ...
  IndexFunction::CombIndexToVecIndex(
